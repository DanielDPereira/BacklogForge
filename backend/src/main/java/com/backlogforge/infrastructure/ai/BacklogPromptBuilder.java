    ...
    public String buildPrompt(GenerateBacklogRequest request, String pdfExtractedText) {
        ...
        if (Boolean.TRUE.equals(request.suggestTechnologies())) {
            if (request.technologies() != null && !request.technologies().isEmpty()) {
                ...
                sb.append("   - FORMATO JSON: O campo `suggestedTechnologies` DEVE ser uma lista/array plano de strings (ex: [\"Java 21\", \"Spring Boot\", \"React\", \"PostgreSQL\"]). NUNCA use um objeto ou mapa de chave-valor como {\"backend\": \"Java\"}.\n");
                ...
                sb.append("   - O usuário solicitou que você sugira a melhor stack tecnológica para o projeto. Analise o escopo e preencha o campo `suggestedTechnologies` com uma lista plana de strings contendo tecnologias modernas e robustas (ex: [\"Java 21\", \"Spring Boot\", \"React\", \"PostgreSQL\"]).\n");
            }
        } else {
            sb.append("   - Tecnologias ESTRITAS especificadas pelo usuário: ").append(formatTechnologies(request.technologies())).append("\n");
            sb.append("   - Mantenha `suggestedTechnologies` contendo exatamente as tecnologias informadas pelo usuário como uma lista de strings. Todas as User Stories e Tasks DEVEM utilizar EXCLUSIVAMENTE essa stack.\n");
        }
